...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        with profile(activities=[ProfilerActivity.CPU], record_shapes=True, profile_memory=True) as p1:
        ...
        with profile(activities=[ProfilerActivity.CPU], record_shapes=True, profile_memory=True) as p3:
            with record_function('inference_efficient_former_l1'):
                efficient_former_l1(patches)

        with profile(activities=[ProfilerActivity.CPU], record_shapes=True, profile_memory=True) as p4:
            with record_function('inference_efficient_former_l3'):
                efficient_former_l3(patches)

        with profile(activities=[ProfilerActivity.CPU], record_shapes=True, profile_memory=True) as p5:
